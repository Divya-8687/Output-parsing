...
openai.api_key = "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"

class OpenAIPydantic:
    def __init__(self) -> None:
        pass
    
    def definePrompt(self,prompt_template_str):
    ...
    def executePrompt(self,program,prompt):
        ...
        return program(query=prompt)
    
class Emotion(BaseModel):
    """Data model for a Emotion."""

    emotion: str
    fraction_confidence: float


class Response(BaseModel):
    """Data model for an Response."""

    client_emotional_state: List[Emotion]
    therapist_emotional_state: List[Emotion]
    
if __name__=="__main__":
    prompt_template_str = """\
    A client has following emotions - anger, fear, sadness, shame, loneliness, nervousness, disappointment,\
    helplessness, guilt, boredom, joy, love, optimism, relief, calm. A therapist has the following emotions\
    - supportive, empathetic, compassionate, calm, kind, bored, frustrated, irritated, sad, angry. Assume that\
    only these emotions exist for the following analysis. Assign top three emotions to the patient and the\ 
    therapist, only from the list provided, along with a fractional confidence. Do a sentiment analysis on\
        following dialogue - \n{query}
    """
    openAIPydantic = OpenAIPydantic()
    program = openAIPydantic.definePrompt(prompt_template_str)
    ...
    output = openAIPydantic.executePrompt(program,dialog_query)
